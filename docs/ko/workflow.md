...
dLazy PPT의 워크플로는 단계별 확인을 중요하게 생각합니다. 처음부터 전체 PPT를 바로 생성하지 않고 개요, 스타일과 샘플 슬라이드를 먼저 확인해 재작업 비용을 줄입니다.
...
모든 슬라이드 이미지는 `scripts/image_gen.py`를 통해 dLazy에서 생성됩니다. 첫 이미지를 생성하기 전에 agent가 `dlazy_ppt_runtime.py doctor --check-api`로 key 설정과 모델 사용 가능 여부를 확인합니다.

이 단계는 선택이 아니라 준비 상태 점검입니다. 다른 이미지 백엔드는 없으며, agent가 자체 내장 이미지 도구로 대체해서도 안 됩니다.
...
샘플 슬라이드 확인 후 agent는 각 페이지의 `origin_image/slide_XX.png`를 생성합니다. 하위 agent를 지원하는 환경에서는 각 하위 agent가 한 페이지씩 병렬로 생성해 여러 페이지의 제작 속도를 높입니다. 모든 페이지는 승인된 샘플 슬라이드와 동일한 스타일 및 동일한 생성 파라미터를 사용합니다.
