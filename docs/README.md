# dLazy PPT Skill 说明文档

dLazy PPT 是一个面向 Codex 的 PPT 生成 skill，也可在 Claude Code、OpenClaw、Hermes Agent 等支持 `SKILL.md` 的 agent 中使用。它把文章、报告、论文、课程笔记或粗略想法转换成图片式演示文稿：先规划大纲和视觉风格，再逐页生成完整幻灯片图片，最后组装成 `.pptx` 文件。
...
- [安装与配置](installation.md)：Codex、OpenClaw、Claude Code、Hermes Agent 的安装与更新方式，以及 dLazy API key 配置。
- [标准工作流](workflow.md)：从大纲确认、风格确认、样张确认到整套生成和组装的完整流程。
...
- 可沉淀个人风格库：满意的风格可以保存到 `~/.dlazy-ppt/references/`，存放在 skill 安装目录之外，更新 skill 不丢失，后续制作直接按名字复用。
...
dLazy PPT 生成的是图片式 PPT：视觉一致性强，但页面里的文字、图表和形状不能像传统 PPT 那样逐项编辑。

如果每个文本框和图表都必须单独可编辑，请改用传统 PPT 工具，这个 skill 不适用。

生图需要一个 dLazy API key，参见[安装与配置](installation.md)。

## 相关链接

- GitHub 仓库：https://github.com/dlazyai/ai-ppt-slides
- ClawHub 页面：https://clawhub.ai/dlazyai/dlazy-ppt
- 提交问题：https://github.com/dlazyai/ai-ppt-slides/issues/new
- dLazy 控制台：https://dlazy.com/dashboard/organization/api-key
