#!/usr/bin/env python3
"""Record and copy a dlazy-ppt slide image generated by a subagent."""
...
def _validate_backend(backend: str) -> str:
    ...
    if "image_gen" not in lowered:
        raise SystemExit(
            "--backend-used must name the image tool that produced the slide: "
...
def _backend_family(value: str) -> str:
    """Group equivalent spellings of the one supported image tool.

    Workers label the tool slightly differently (`image_gen.py`,
    `scripts/image_gen.py`), and those must still match the label recorded for
    the approved sample. Anything else is not a family, it is a different tool.
    """
    if "image_gen" in value.lower():
        return "dlazy-image-cli"
    return ""
